...
def load_real_samples_min_n_max(file_path, training_keys, validation_keys, testing_keys):
    ...
    all_keys = [float(i) for i in data_dict.keys()]
    print(all_keys)
    max_key = np.max(np.asarray(all_keys))
    min_key = np.min(np.asarray(all_keys))
    # training_keys = all_keys[0:2] + all_keys[3:-4] + all_keys[-3:]
    # validation_keys = [all_keys[2]]
    # testing_keys = [all_keys[-4]]
    ...
    for key in training_keys:
        ...
        training_data.append(np.array(data_dict[temp_key])[:, -1])
        for i in range(np.array(data_dict[temp_key]).shape[0]):
            normed_key = 2 * (key - min_key) / (max_key - min_key) - 1.0
            training_codes.append(normed_key)
    ...
    for key in validation_keys:
        ...
        for i in range(np.array(data_dict[temp_key]).shape[0]):
            normed_key = 2 * (key - min_key) / (max_key - min_key) - 1.0
            validation_codes.append(normed_key)
    ...
    for key in testing_keys:
        ...
        for i in range(np.array(data_dict[temp_key]).shape[0]):
            normed_key = 2 * (key - min_key) / (max_key - min_key) - 1.0
            testing_codes.append(normed_key)
    ...
    testing_data = (2 * (testing_data - np.min(training_data, axis=0)) / (
            np.max(training_data, axis=0) - np.min(training_data, axis=0)) - 1.0) * 0.8
    training_data_norm = np.copy(training_data)
    training_data_norm = (2 * (training_data - np.min(training_data, axis=0)) / (
            np.max(training_data, axis=0) - np.min(training_data, axis=0)) - 1.0) * 0.8
    ...
    return training_data_norm, training_codes_norm, validation_data, validation_codes_norm, testing_data, testing_codes_norm, training_data_min[
                                                                                                                              :-1], training_data_max[
                                                                                                                                    :-1]
...
def get_real_data_constant_code(real_data, samples_dim, code):
    ...
    for datum in real_data:
        lala = round(datum[samples_dim:][0], 8)
        lala_2 = round(code, 8)
        if lala == lala_2:
...
def test_KL_divergence(generator, noise_dim, code_dim, validation_kernels, val_codes, training_noise_dict,
                       n_samples_per_axis=40, n_axis=1):
    total_KL = 0
    for code in validation_kernels:
        print(code)
        fakes = generate_fake_samples_constant_code(generator, code_dim, noise_dim, code, n_samples_per_axis ** n_axis,
                                                    val_codes, training_noise_dict)
...
def train(generator, discriminator, gan, dataset, available_codes, samples_dim, noise_dim, code_dim,
          training_noise_dict,
    ...
    for i in range(n_epochs):
        ...
        if (i + 1) % evaluate_freq == 0 and (i + 1) > evaluate_after:
            val_KL_divergence = test_KL_divergence(generator, noise_dim, code_dim, validation_kernels, available_codes,
                                                   training_noise_dict, n_axis=samples_dim, n_samples_per_axis=1000)
            print("##############################################################")
...
def test_best_generator(path, sample_dim, code_dim, noise_dim, testing_data, testing_codes_norm_value,
                        validation_codes_norm, training_noise_dict):
    best_gen = load_model(path)
    fakes = generate_fake_samples_constant_code(best_gen, code_dim, noise_dim, testing_codes_norm_value, 1000,
                                                validation_codes_norm, training_noise_dict)
...
def test_best_generator_distribs(path, sample_dim, code_dim, noise_dim, testing_data, testing_codes_norm_value,
                                 validation_codes_norm, train_min, train_max, training_noise_dict):
    best_gen = load_model(path)
...
def CGAN_fit(training_data_norm, training_codes_norm, val_kernels, noise_dim, sample_dim, code_dim, training_noise_dict,
             n_epochs, n_batch, eval_every_epochs, disc_hidden_size, gen_hidden_size):
    ...
    min_kl_divergence, best_generator = train(generator, discriminator, gan, training_data_norm, training_codes_norm,
                                              sample_dim, noise_dim, code_dim,
